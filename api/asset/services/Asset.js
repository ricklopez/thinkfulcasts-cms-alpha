...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Asset.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Asset.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Asset._attributes)
      .filter(attribute => attribute !== Asset.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Asset._attributes[attribute].type));
