...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Asset.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Asset.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Asset.attributes).reduce((acc, curr) => {
      switch (Asset.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
